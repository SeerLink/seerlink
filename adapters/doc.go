...
//
// The seerlink node must know the corresponding secret key. Such a key pair
// can be created with the `seerlink local vrf create` command, and exported to
// a keystore with `vrf export <keystore-path>`.
//
// E.g. `seerlink local vrf create -p <password-file>` will log the public key
// under the field "public id".
//
// To see the public keys which have already been imported, use the command
// `seerlink local vrf list`. See `seerlink local vrf help` for more
