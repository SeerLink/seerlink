...
import { ConcreteSeerlink__factory } from '../../ethers/v0.4/factories/ConcreteSeerlink__factory'

const provider = setup.provider()
const concreteSeerlinkFactory = new ConcreteSeerlink__factory()
const debug = d.makeDebug('ConcreteSeerlink')

describe('ConcreteSeerlink', () => {
  let ccl: contract.Instance<ConcreteSeerlink__factory>
  let defaultAccount: ethers.Wallet
  const deployment = setup.snapshot(provider, async () => {
    defaultAccount = await setup
      .users(provider)
      .then((r) => r.roles.defaultAccount)
    ccl = await concreteSeerlinkFactory.connect(defaultAccount).deploy()
  })
  ...
  it('has a limited public interface', () => {
    matchers.publicAbi(concreteSeerlinkFactory, [
      'add',
      'addBytes',
      'addInt',
      'addStringArray',
