...
import { ConcreteSeerlinked__factory } from '../../ethers/v0.4/factories/ConcreteSeerlinked__factory'
...
const concreteSeerlinkedFactory = new ConcreteSeerlinked__factory()
...
describe('ConcreteSeerlinked', () => {
  const specId =
    '0x4c7b7ffb66b344fbaa64995af81e355a00000000000000000000000000000000'
  let cc: contract.Instance<ConcreteSeerlinked__factory>
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    cc = await concreteSeerlinkedFactory
      .connect(roles.defaultAccount)
  ...
  describe('#seerlinkRequest(Request)', () => {
    it('emits an event from the contract showing the run ID', async () => {
      ...
      assert.equal(events?.[0].event, 'SeerlinkRequested')
    })
  })

  describe('#seerlinkRequestTo(Request)', () => {
    it('emits an event from the contract showing the run ID', async () => {
  ...
  describe('#cancelSeerlinkRequest', () => {
    let requestId: string
    // a concrete seerlink attached to an empty oracle
    let ecc: contract.Instance<ConcreteSeerlinked__factory>

    beforeEach(async () => {
      const emptyOracle = await emptyOracleFactory
        .connect(roles.defaultAccount)
        .deploy()
      ecc = await concreteSeerlinkedFactory
        .connect(roles.defaultAccount)
    ...
    it('emits an event from the contract showing the run was cancelled', async () => {
      ...
      assert.equal(events?.[0].event, 'SeerlinkCancelled')
  ...
  describe('#recordSeerlinkFulfillment(modifier)', () => {
    ...
    it('emits an event marking the request fulfilled', async () => {
      ...
      assert.equal(event?.name, 'SeerlinkFulfilled')
      assert.equal(request.requestId, event?.values.id)
    })
  })

  describe('#fulfillSeerlinkRequest(function)', () => {
    let request: oracle.RunRequest
    ...
    beforeEach(async () => {
      const tx = await cc.publicRequest(
        ...
        ethers.utils.toUtf8Bytes(
          'publicFulfillSeerlinkRequest(bytes32,bytes32)',
        ),
        0,
      )
      const { logs } = await tx.wait()
  ...
  describe('#seerlinkToken', () => {
    it('returns the Link Token address', async () => {
      const addr = await cc.publicSeerlinkToken()
      assert.equal(addr, link.address)
    })
  })

  describe('#addExternalRequest', () => {
    let mock: contract.Instance<ConcreteSeerlinked__factory>
    let request: oracle.RunRequest

    beforeEach(async () => {
      mock = await concreteSeerlinkedFactory
        .connect(roles.defaultAccount)
        .deploy(link.address, oc.address)
