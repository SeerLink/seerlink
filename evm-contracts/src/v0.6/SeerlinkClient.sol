...
contract SeerlinkClient {
   ...
   */
  function sendSeerlinkRequest(Seerlink.Request memory _req, uint256 _payment)
    internal
    returns (bytes32)
  {
    return sendSeerlinkRequestTo(address(oracle), _req, _payment);
  }
   ...
   * @return requestId The request ID
   */
  function sendSeerlinkRequestTo(address _oracle, Seerlink.Request memory _req, uint256 _payment)
    internal
  ...
  function cancelSeerlinkRequest(
    ...
    internal
  {
    SeerlinkRequestInterface requested = SeerlinkRequestInterface(pendingRequests[_requestId]);
    delete pendingRequests[_requestId];
    emit SeerlinkCancelled(_requestId);
   ...
   */
  function setSeerlinkOracle(address _oracle) internal {
    oracle = SeerlinkRequestInterface(_oracle);
  }
   ...
   */
  function setSeerlinkToken(address _link) internal {
   ...
   */
  function addSeerlinkExternalRequest(address _oracle, bytes32 _requestId)
    internal
   ...
   */
  function useSeerlinkWithENS(address _ens, bytes32 _node)
    internal
  {
   ...
   */
  function encodeRequest(Seerlink.Request memory _req)
    private
    view
   ...
   */
  function validateSeerlinkCallback(bytes32 _requestId)
    internal
   ...
   * @param _requestId The request ID for fulfillment
   */
  modifier recordSeerlinkFulfillment(bytes32 _requestId) {
    require(msg.sender == pendingRequests[_requestId],
            "Source must be the oracle of the request");
    delete pendingRequests[_requestId];
    emit SeerlinkFulfilled(_requestId);
    _;
  }
