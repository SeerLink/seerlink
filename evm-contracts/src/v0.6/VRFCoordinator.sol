...
contract VRFCoordinator is VRF, VRFRequestIDBase {
  ...
  struct ServiceAgreement { // Tracks oracle commitments to VRF service
    ...
    bytes32 jobID; // ID of corresponding seerlink job in oracle's DB
  }
   ...
   * @param _oracle the address of the Seerlink node with the proving key and job
   * @param _publicProvingKey public key used to prove randomness
   * @param _jobID ID of the corresponding seerlink job in the oracle's db
   */
  ...
  /**
   * @notice creates the seerlink request for randomness
   *
