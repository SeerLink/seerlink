 ...
 */
contract PreCoordinator is SeerlinkClient, Ownable, SeerlinkRequestInterface, LinkTokenReceiver {
   ...
   * @dev This is the public implementation for seerlinkTokenAddress, which is
   * an internal method of the SeerlinkClient contract
   */
  function getSeerlinkToken() public view override returns (address) {
    return seerlinkTokenAddress();
  }

  /**
   * @notice Creates the Seerlink request
   * @dev Stores the hash of the params as the on-chain commitment for the request.
   * Emits OracleRequest event for the Seerlink node to detect.
  ...
  /**
   * @dev Creates Seerlink requests to each oracle in the service agreement with the
  ...
  function createRequests(bytes32 _saId, bytes32 _incomingRequestId, bytes memory _data) private {
    ...
    for (uint i = 0; i < sa.oracles.length; i++) {
      request = buildSeerlinkRequest(sa.jobIds[i], address(this), this.seerlinkCallback.selector);
      request.setBuffer(_data);
      outgoingRequestId = sendSeerlinkRequestTo(sa.oracles[i], request, sa.payments[i]);
   ...
   * @dev The recordSeerlinkFulfillment protects this function from being called
   * by anyone other than the oracle address that the request was sent to
   * @param _requestId The ID that was generated for the request
   * @param _data The answer provided by the oracle
   */
  function seerlinkCallback(bytes32 _requestId, int256 _data)
    external
  ...
  modifier checkCallbackAddress(address _to) {
    require(_to != seerlinkTokenAddress(), "Cannot callback to LINK");
    _;
  }
}
