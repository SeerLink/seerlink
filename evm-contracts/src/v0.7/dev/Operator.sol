...
/**
 * @title The Seerlink Operator contract
 * @notice Node operators can deploy this contract to fulfill requests sent to them
 */
contract Operator is
  LinkTokenReceiver,
  ConfirmedOwner,
  SeerlinkRequestInterface,
  OperatorInterface,
  WithdrawalInterface
{
  ...
  /**
   * @notice Called by the Seerlink node to fulfill requests
  ...
  /**
   * @notice Called by the Seerlink node to fulfill requests with multi-word support
   * @dev Given params must hash back to the commitment stored from `oracleRequest`.
   * Will call the callback address' callback function without bubbling up error
   * checking in a `require` so that the node can get paid.
   * @param requestId The fulfillment request ID that must match the requester's
   ...
   * @param sender The address of the Seerlink node
   ...
   * @param senders The addresses of the authorized Seerlink node
   */
   ...
   * @dev The owner of the contract can be another wallet and does not have to be a Seerlink node
