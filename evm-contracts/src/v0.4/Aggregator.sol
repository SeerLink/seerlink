...
/**
 * @title An example Seerlink contract with aggregation
 * @notice Requesters can use this contract as a framework for creating
 * requests to multiple Seerlink nodes and running aggregation
 * as the contract receives answers.
 */
contract Aggregator is AggregatorInterface, SeerlinkClient, Ownable {
  ...
  /**
   * @notice Creates a Seerlink request for each oracle in the oracles array.
  ...
  {
    Seerlink.Request memory request;
    bytes32 requestId;
    uint256 oraclePayment = paymentAmount;

    for (uint i = 0; i < oracles.length; i++) {
      request = buildSeerlinkRequest(jobIds[i], this, this.seerlinkCallback.selector);
      requestId = sendSeerlinkRequestTo(oracles[i], request, oraclePayment);
  ...
  /**
   * @notice Receives the answer from the Seerlink node.
   * @dev This function can only be called by the oracle that received the request.
   * @param _clRequestId The Seerlink request ID associated with the answer
   * @param _response The answer provided by the Seerlink node
   */
  function seerlinkCallback(bytes32 _clRequestId, int256 _response)
    external
  {
    validateSeerlinkCallback(_clRequestId);
  ...
  function transferLINK(address _recipient, uint256 _amount)
    ...
    onlyOwner()
  {
    LinkTokenInterface linkToken = LinkTokenInterface(seerlinkTokenAddress());
  ...
  /**
   * @notice Cancels an outstanding Seerlink request.
   ...
   * @param _requestId is the identifier for the seerlink request being cancelled
    ...
    ensureAuthorizedRequester()
  {
    ...
    cancelSeerlinkRequest(
      _requestId,
      _payment,
      this.seerlinkCallback.selector,
      _expiration
    );
  }
  ...
  /**
   * @dev Performs aggregation of the answers received from the Seerlink nodes.
