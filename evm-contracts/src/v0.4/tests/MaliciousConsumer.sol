...
contract MaliciousConsumer is Seerlinked {
  ...
  function requestData(bytes32 _id, bytes _callbackFunc) public {
    Seerlink.Request memory req = newRequest(_id, this, bytes4(keccak256(_callbackFunc)));
    expiration = now.add(5 minutes); // solhint-disable-line not-rely-on-time
    seerlinkRequest(req, ORACLE_PAYMENT);
  }
  ...
  function stealEthCall(bytes32 _requestId, bytes32) public recordSeerlinkFulfillment(_requestId) {
    require(address(this).call.value(100)(), "Call failed"); // solhint-disable-line avoid-call-value
  }

  function stealEthSend(bytes32 _requestId, bytes32) public recordSeerlinkFulfillment(_requestId) {
    // solhint-disable-next-line check-send-result
    require(address(this).send(100), "Send failed"); // solhint-disable-line multiple-sends
  }

  function stealEthTransfer(bytes32 _requestId, bytes32) public recordSeerlinkFulfillment(_requestId) {
