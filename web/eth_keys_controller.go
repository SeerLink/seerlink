...
// Index returns the node's Ethereum keys and the account balances of ETH & SEER.
// Example:
...
func (ekc *ETHKeysController) Index(c *gin.Context) {
	...
	for _, key := range keys {
		...
		linkBalance, err := store.EthClient.GetSEERBalance(linkAddress, key.Address.Address())
		if err != nil {
			err = errors.Errorf("error calling getSEERBalance on Ethereum node: %v", err)
			jsonAPIError(c, http.StatusInternalServerError, err)
			return
		}
...
func (ekc *ETHKeysController) Create(c *gin.Context) {
	...
	linkBalance, err := ekc.App.GetStore().EthClient.GetSEERBalance(linkAddress, account.Address)
	if err != nil {
		logger.Errorf("error calling getSEERBalance on Ethereum node: %v", err)
	}
...
func (ekc *ETHKeysController) Delete(c *gin.Context) {
	...
	linkAddress := common.HexToAddress(ekc.App.GetStore().Config.LinkContractAddress())
	linkBalance, err := ekc.App.GetStore().EthClient.GetSEERBalance(linkAddress, address)
	if err != nil {
		logger.Errorf("error calling getSEERBalance on Ethereum node: %v", err)
	}
...
func (ekc *ETHKeysController) Import(c *gin.Context) {
	...
	linkAddress := common.HexToAddress(ekc.App.GetStore().Config.LinkContractAddress())
	linkBalance, err := store.EthClient.GetSEERBalance(linkAddress, acct.Address)
	if err != nil {
		err = errors.Errorf("error calling getSEERBalance on Ethereum node: %v", err)
		jsonAPIError(c, http.StatusInternalServerError, err)
		return
