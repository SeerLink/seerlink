...
func (n SeerlinkAppFactory) NewApplication(config *orm.Config, onConnectCallbacks ...func(seerlink.Application)) seerlink.Application {
	...
	return seerlink.NewApplication(config, ethClient, advisoryLock, store.StandardKeyStoreGen, services.NewExternalInitiatorManager(), onConnectCallbacks...)
}
...
func (n SeerlinkRunner) Run(app seerlink.Application) error {
	gin.SetMode(app.GetStore().Config.LogLevel().ForGin())
	handler := web.Router(app.(*seerlink.SeerlinkApplication))
...
func (c changePasswordPrompter) Prompt() (models.ChangePasswordRequest, error) {
	fmt.Println("Changing your seerlink account password.")
