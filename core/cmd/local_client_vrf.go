...
import (
	...
	"github.com/SeerLink/seerlink/core/store/models/vrfkey"
...
Compressed public key (use this for interactions with the seerlink node):
  %s
...
seerlink local vrf export -f <save_path> -pk %s
...
`,
				fmt.Sprintf("seerlink local delete -pk %s", key.PublicKey),
				fmt.Sprintf("`seerlink local export -f <backup_path> -pk %s`",
					key.PublicKey))
...
//
// Since this runs in an independent process from any seerlink node, it cannot
