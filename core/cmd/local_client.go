...
func (cli *Client) RunNode(c *clipkg.Context) error {
	...
	logger.Infow("Starting Seerlink Node " + static.Version + " at commit " + static.Sha)

	app := cli.AppFactory.NewApplication(cli.Config, func(app seerlink.Application) {
		store := app.GetStore()
...
func loggedStop(app seerlink.Application) {
...
func checkFilePermissions(rootDir string) error {
	...
	for _, fileName := range protectedFiles {
		...
		owned, err := utils.IsFileOwnedBySeerlink(fileInfo)
		if err != nil {
			logger.Warn(err)
			continue
		}
		if !owned {
			logger.Warnf("The file %v is not owned by the user running seerlink. This will be made mandatory in the future.", path)
		}
	}
	return nil
}
...
func logIfNonceOutOfSync(store *strpkg.Store, key models.Key) {
	...
	if nonce < int64(onChainNonce) {
		logger.Warn(fmt.Sprintf("The account %s is being used by another wallet and is not safe to use with seerlink", key.Address.Hex()))
	}
}
...
func (cli *Client) HardReset(c *clipkg.Context) error {
	...
	fmt.Print("Do not run this while a Seerlink node is currently using the DB as it could cause undefined behavior.\n\n")
	if !confirmAction(c) {
		return nil
	}
...
func (cli *Client) makeApp() (seerlink.Application, func()) {
	app := cli.AppFactory.NewApplication(cli.Config)
	return app, func() {
...
func (cli *Client) ResetDatabase(c *clipkg.Context) error {
	...
	if config.DatabaseURL() == "" {
		return cli.errorOut(errors.New("You must set DATABASE_URL env variable. HINT: If you are running this to set up your local test database, try DATABASE_URL=postgresql://postgres@localhost:5432/seerlink_test?sslmode=disable"))
	}
