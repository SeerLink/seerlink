...
import (
	...
	ocr "github.com/SeerLink/libocr/offchainreporting"
...
// ClientNodeURL is the URL of the Ethereum node this Seerlink node should connect to.
...
// DatabaseURL configures the URL for seerlink to connect to. This must be
...
// Dev configures "development" mode for seerlink.
...
// EthereumURL represents the URL of the Ethereum node to connect Seerlink to.
...
// LogSQLStatements tells seerlink to log all SQL statements made using the default logger
func (c Config) LogSQLStatements() bool {
	return c.viper.GetBool(EnvVarName("LogSQLStatements"))
}

// LogSQLMigrations tells seerlink to log all SQL migrations made using the default logger
...
// Port represents the port Seerlink should listen on for client requests.
...
// RootDir represents the location on the file system where Seerlink should
// keep its files.
...
// TLSKeyPath represents the file system location of the TLS key Seerlink
// should use for HTTPS.
func (c Config) TLSKeyPath() string {
	return c.viper.GetString(EnvVarName("TLSKeyPath"))
}

// TLSPort represents the port Seerlink should listen on for encrypted client requests.
