...
func CheckSquashUpgrade(db *gorm.DB) error {
	...
	if squashVersionMinus1.LessThan(*currentVersion) {
		...
		if q.RowsAffected == 0 {
			...
			return errors.Errorf("Need to upgrade to seerlink version %v first before upgrading to version %v in order to run migrations", squashVersionMinus1, currentVersion)
		}
	}
	return nil
}
