...
// SeerlinkFulfilledTopic is the signature for the event emitted after calling
// SeerlinkClient.validateSeerlinkCallback(requestId). See
// ../../evm-contracts/src/v0.6/SeerlinkClient.sol
var SeerlinkFulfilledTopic = utils.MustHash("SeerlinkFulfilled(bytes32)")
...
func ReceiptIndicatesRunLogFulfillment(txr types.Receipt) bool {
	for _, log := range txr.Logs {
		if log.Topics[0] == SeerlinkFulfilledTopic {
			return true
		}
	}
	return false
