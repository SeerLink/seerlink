...
import (
	...
	"github.com/SeerLink/seerlink/core/internal/gethwrappers/generated/flags_wrapper"
	faw "github.com/SeerLink/seerlink/core/internal/gethwrappers/generated/flux_aggregator_wrapper"
...
func awaitSubmission(t *testing.T, submissionReceived chan *faw.FluxAggregatorSubmissionReceived) (
	...
	select { // block until FluxAggregator contract acknowledges seerlink message
	case log := <-submissionReceived:
		return log.Raw.BlockNumber, log.Submission.Int64()
	case <-time.After(10 * pollTimerPeriod):
		t.Fatalf("seerlink failed to submit answer to FluxAggregator contract")
...
func TestFluxMonitorAntiSpamLogic(t *testing.T) {
	...
	// Response by malicious seerlink node, nallory
	...
	case <-submissionReceived:
		t.Fatalf("FA allowed seerlink node to start a new round early")
	...
	assert.Error(t, err, "FA allowed seerlink node to start a new round early")
...
func TestFluxMonitor_HibernationMode(t *testing.T) {
	...
	require.NoError(t, app.StartAndConnect(), "failed to start seerlink")
