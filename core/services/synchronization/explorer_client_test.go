...
func TestWebSocketClient_Authentication(t *testing.T) {
	...
	cltest.CallbackOrTimeout(t, "receive authentication headers", func() {
		...
		assert.Equal(t, []string{"accessKey"}, headers["X-Explore-Seerlink-Accesskey"])
		assert.Equal(t, []string{"secret"}, headers["X-Explore-Seerlink-Secret"])
		assert.Equal(t, []string{static.Version}, headers["X-Explore-Seerlink-Core-Version"])
		assert.Equal(t, []string{static.Sha}, headers["X-Explore-Seerlink-Core-Sha"])
	})
}
