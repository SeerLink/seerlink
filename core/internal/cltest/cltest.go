...
import (
	...
	"github.com/SeerLink/seerlink/core/static"

	p2ppeer "github.com/libp2p/go-libp2p-core/peer"
	"github.com/SeerLink/seerlink/core/assets"
	"github.com/SeerLink/seerlink/core/auth"
	"github.com/SeerLink/seerlink/core/cmd"
	"github.com/SeerLink/seerlink/core/gracefulpanic"
	"github.com/SeerLink/seerlink/core/internal/gethwrappers/generated/flux_aggregator_wrapper"
	...
	"github.com/SeerLink/seerlink/core/services/eth/contracts"
	"github.com/SeerLink/seerlink/core/services/pipeline"
	"github.com/SeerLink/seerlink/core/services/postgres"
	strpkg "github.com/SeerLink/seerlink/core/store"
	...
	ocrtypes "github.com/SeerLink/libocr/offchainreporting/types"
...
const (
	...
	APIEmail = "apiuser@seerlink.test"
...
func init() {
	...
	if err != nil || parsed.Path == "" {
		msg := fmt.Sprintf("invalid DATABASE_URL: `%s`. You must set DATABASE_URL env var to point to your test database. Note that the test database MUST end in `_test` to differentiate from a possible production DB. HINT: Try DATABASE_URL=postgresql://postgres@localhost:5432/seerlink_test?sslmode=disable", config.DatabaseURL())
		panic(msg)
	}
	if !strings.HasSuffix(parsed.Path, "_test") {
		msg := fmt.Sprintf("cannot run tests against database named `%s`. Note that the test database MUST end in `_test` to differentiate from a possible production DB. HINT: Try DATABASE_URL=postgresql://postgres@localhost:5432/seerlink_test?sslmode=disable", parsed.Path[1:])
		panic(msg)
	}
...
func NewTestConfig(t testing.TB, options ...interface{}) *TestConfig {
	...
	rawConfig.Set("SEERLINK_DEV", true)
...
func NewApplicationWithConfig(t testing.TB, tc *TestConfig, flagsAndDeps ...interface{}) (*TestApplication, func()) {
	...
	var externalInitiatorManager seerlink.ExternalInitiatorManager = &services.NullExternalInitiatorManager{}
	for _, flag := range flagsAndDeps {
		...
		case postgres.AdvisoryLocker:
			advisoryLocker = dep
		case seerlink.ExternalInitiatorManager:
	...
	app := seerlink.NewApplication(tc.Config, ethClient, advisoryLocker, strpkg.InsecureKeyStoreGen, externalInitiatorManager, func(app seerlink.Application) {
		ta.connectedChannel <- struct{}{}
	}).(*seerlink.SeerlinkApplication)
	ta.SeerlinkApplication = app
	server := newServer(ta)
...
func newServer(app seerlink.Application) *httptest.Server {
...
func (ta *TestApplication) Start() error {
	...
	err := ta.SeerlinkApplication.Start()
	return err
}
...
func (ta *TestApplication) Stop() error {
	...
	require.NoError(ta.t, ta.SeerlinkApplication.StopIfStarted())
...
func (ta *TestApplication) NewClientAndRenderer() (*cmd.Client, *RendererMock) {
	...
	client := &cmd.Client{
		...
		AppFactory:                     seededAppFactory{ta.SeerlinkApplication},
...
// MockApplicationEthCalls mocks all calls made by the seerlink application as
