...
// chainIDVerify checks whether or not the ChainID from the Seerlink config
// matches the ChainID reported by the ETH node connected to this Seerlink node.
func verifyEthereumChainID(ht *HeadTracker) error {
	...
	if ethereumChainID.Cmp(ht.store.Config.ChainID()) != 0 {
		return fmt.Errorf(
			"ethereum ChainID doesn't match seerlink config.ChainID: config ID=%d, eth RPC ID=%d",
